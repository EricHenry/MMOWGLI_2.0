 ...
 *
 * This class creates a container that holds and displays the different components of the MMOWGLI main page.
 * 
 */
...
public class HomePage extends CustomComponent {
	...
	public HomePage() {
		...
		initPlayerHistoryList();
		
		//set Listeners
		setSearchButtonListeners();
		setActivePathListener();
	}
	...
	private void initTrendingTags(){
		...
		try {
			...
			while(topTags.size() > 0){
				...
				//Style the link
				homePageUpdates.getTrendingTags().getLink_container().setComponentAlignment(tag, Alignment.MIDDLE_LEFT);
			}
	...
	public void initPopularTags(){
		...
		try {
			...
			while(topTags.size() > 0){
				...
				//Style the link
				homePageUpdates.getPopularTags().getLink_container().setComponentAlignment(tag, Alignment.MIDDLE_LEFT);
			}
	...
	/**
	 * This method gets the last 5 cards that the user has clicked and 
	 * 	displays them in the updater section of the homepage.
	 */
	private void initPlayerHistoryList(){
		try {
			//get the list from the database
			CardList history = MmowgliDB.playerHistoryQuery(1);
			...
			while(history.size() > 0){
				...
				//add card to the view
				homePageUpdates.getPlayerHistory().getMainLayout().addComponent(newCard);
				homePageUpdates.getPlayerHistory().getMainLayout().setComponentAlignment(newCard, Alignment.MIDDLE_CENTER);
			}
	...
	/**
	 *  Set the default click listeners for BaseCards 
	 *  	that are displayed in the Updates section
	 * 
	 * @param baseCard	-> Base Card displayed in the home page updater
	 */
		...
		baseCard.getNativeButton_text().addClickListener(new NativeButton.ClickListener(){
			...
			public void buttonClick(ClickEvent event){
				...
				try {
					...
					//update the database of a clicked card
					MmowgliDB.addPlayerHistoryQuery(data.cardId, 1);
	...
	/**
	 *  Set the default click listeners for Native Button tags
	 *  	that are displayed in the home page updater section
	 *  
	 * @param tag	-> Native button tags displayed in the home page updater
	 */
	...
	/**
	 * Sets the listener to search for what was entered in the 
	 * 	search bar. Then displays the cards in the card explorer.
	 * 	Right now it is only able to search for tags.
	 */
	public void setSearchButtonListeners(){
		
		sideBar.getNativeButton_Search().addClickListener(new NativeButton.ClickListener() {
		      
			public void buttonClick(ClickEvent event) {
		    	  //tagSearch();
				String tag = sideBar.getTextField_search().getValue();
				
				if(!tag.equalsIgnoreCase("") && !tag.equalsIgnoreCase(" ") && !tag.equalsIgnoreCase(null)){
					try {
						panel_2.setScrollTop(1000);
						
						CardList tags = MmowgliDB.taggedCardsQuery(tag);
						
						//clear the card explorer
						cardExplorationView.clearExplorationView();
						
						cardExplorationView.displayCardList(tags, cardExplorationView.getVerticalLayout_mainView());
	...
	/**
	 * Sets the listener to, when clicked, show the best path 
	 * 	from the card that is currently chosen.
	 */
	public void setActivePathListener(){
		
		sideBar.getnButton_path().addClickListener(new NativeButton.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				try {
					...
					Card chosenCard = MmowgliDB.oneCardQuery(cardId);
					
					//get the active path from DB
					//activePath = BestPathManager.activePath(cardId, activePath);
					activePath = BestPathManager.activePath(chosenCard, activePath);
					
					//scroll down to the Card explorer
					panel_2.setScrollTop(1000);
					
					//clear the children form the card explorer
					cardExplorationView.getVerticalLayout_childView().removeAllComponents();
					
					//display the active path
					cardExplorationView.displayBestCardPathList(activePath, cardExplorationView.getVerticalLayout_childView());
	...
	private GridLayout buildGridLayout_content() {
		...
		gridLayout_content.setHeight("103.75em");
		gridLayout_content.setMargin(false);
		gridLayout_content.setRows(5);
		...
		// rootCards
		rootCards = new RootCards();
		rootCards.setImmediate(false);
		rootCards.setWidth("100.0%");
		rootCards.setHeight("-1px");
		gridLayout_content.addComponent(rootCards, 0, 1);
		gridLayout_content.setComponentAlignment(rootCards, new Alignment(48));
		...
		gridLayout_content.addComponent(cardExplorationView, 0, 2);
		...
		gridLayout_content.addComponent(footer, 0, 3);
